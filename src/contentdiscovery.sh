...
then
  ...
  python3 ~/tools/dirsearch/dirsearch.py -u https://$DOM -e php,asp,aspx,jsp,py,txt,conf,config,bak,backup,swp,old,db,zip,sql -w ~/tools/wordlists/common.txt -x 429,400,503 -f --random-agents -t 300 --max-retries=3 -b -r
else
  ...
  python3 ~/tools/dirsearch/dirsearch.py -u https://$DOM -e php,asp,aspx,jsp,py,txt,conf,config,bak,backup,swp,old,db,zip,sql -w ~/tools/wordlists/common.txt -x 429,400,503 -f --random-agents -t 300 --max-retries=3 -b -r
  mv ~/tools/dirsearch/reports/$DOM/* ~/recon/$DOM/Content_discovery/dirsearch_result.txt
fi
echo "${yellow} ---------------------------------- xxxxxxxx ---------------------------------- ${reset}"
echo " "
